...
useEffect(() => {
  const delay = setTimeout(() => {
    setCurrentPage(1); // সার্চ পরিবর্তন হলে প্রথম পেজে ফিরে যাবে
  ...
  }, 400); // 400ms debounce — প্রতি অক্ষরে API call হবে না
...
const handleAddBrand = async () => {
 ...
 const fetchProducts = async (page = 1, limit = pageSize) => {
  try {
    setLoading(true);          // ⬅️ API call এর আগেই ON

    const res = await axios.get(
      `https://dailyshopping-backend.onrender.com/api/products/latestproduct?page=${page}&limit=${limit}`
    );
    ...
    setLoading(false);         // ⬅️ data আসার পর OFF
  }
};
...
<div>
  ...
  {/* Product Wise Discount - editable, only shows in edit mode */}
<div className="flex flex-col">
  <label className="font-medium mb-1">Product Wise Discount (%)</label>
  <input
    type="number"
    placeholder="Product Wise Discount"
    className="border rounded-lg p-2"
    value={form.productwiseDiscount}
    onChange={(e) => setForm({ ...form, productwiseDiscount: e.target.value })}
  />
</div>
  ...
  {loading ? (
    ...
    filteredProducts.map((p) => (
      <tr key={p._id} className="hover:bg-gray-50">
        ...
        <td className="border p-2">{p.productwiseDiscount || 0}</td>
